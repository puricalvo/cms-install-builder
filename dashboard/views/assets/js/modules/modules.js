  ...
  $("#myModule").on("shown.bs.modal", function () {
    ...
    if (item != undefined) {
      ...
      if (JSON.parse(item).type_module == "metrics") {
        ...
        $("#metricTable").val(
          JSON.parse(JSON.parse(item).content_module).table,
        );
        $("#metricColumn").val(
          JSON.parse(JSON.parse(item).content_module).column,
        );
        $("#metricConfig").val(
          JSON.parse(JSON.parse(item).content_module).config,
        );
        $("#metricIcon").val(JSON.parse(JSON.parse(item).content_module).icon);
        $("#metricColor").val(
          JSON.parse(JSON.parse(item).content_module).color,
        );
...
$(document).on("click", ".deleteModule", function () {
  ...
  fncSweetAlert("confirm", "¿Está seguro de borrar este módulo?", "").then(
    (resp) => {
      if (resp) {
...
$(document).on("click", ".deleteColumn", function () {
  ...
  fncSweetAlert("confirm", "¿Está seguro de borrar esta columna?", "").then(
    (resp) => {
      if (resp) {
