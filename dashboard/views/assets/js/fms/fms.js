...
function uploadFiles(event, type, time) {
  ...
  Array.from(files.files).forEach((file, i) => {
    if (
      ...
      fncToastr(
        "error",
        "El formato de archivo que intenta subir no es permitido",
      );
      return;
    }
  });
...
$(document).on("click", "#startAll", function () {
  ...
  if ($(".itemsUp").length == 0) {
    fncToastr(
      "error",
      "Debe arrastrar mínimo un archivo para realizar esta acción",
    );
    return;
  }
    ...
    $.ajax({
        ...
        xhr.upload.onprogress = function (e) {
          if (e.lengthComputable) {
            ...
            $(".progressList" + i)
              ...
              .html(
                `<div class="spinner-border spinner-border-sm me-1"></div><small>Uploading file to server...</small>`,
              );

            $(".progressList" + i)
              .find(".progress-bar")
              .attr("style", "width:" + completePercent.toFixed(2) + "%");

            $(".progressList" + i)
              .find(".progress-bar")
              .html(completePercent.toFixed(2) + "%");
      ...
      success: function (response) {
        if (JSON.parse(response).status == 200) {
          ...
          $(".columnName" + i)
            .find("input")
            .attr("idFile", JSON.parse(response).id_file);
          $(".columnName" + i).removeClass("columnName" + i);

          $(".progressList" + i)
            .html(`<a href="${JSON.parse(response).link}" target="_blank">
          ...
          $(".columnAction" + i)
            .html(`<svg class="bi bi-copy copyLink" copy="${JSON.parse(response).link}" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" style="cursor:pointer">
            ...
            $.ajax({
              ...
              success: function (response) {
                if (response == 200) {
                  ...
                  fncToastr(
                    "success",
                    "Todos los archivos han subido exitosamente",
                  );
...
$(document).on("click", ".clearFile", function () {
  ...
  if (mode == "list") {
    $(this).parent().parent().remove();
    $(".clearFile[index='" + index + "'][name='" + name + "']")
      .parent()
      .parent()
      .parent()
      .parent()
